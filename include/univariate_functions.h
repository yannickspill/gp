#ifndef UNIVARIATE_FUNCTIONS_H
#define UNIVARIATE_FUNCTIONS_H
...
class UnivariateFunction {
  public:
    ...
    virtual Eigen::VectorXd operator()(const IMP::MatrixXd& xlist) const = 0;

    //! used for testing only
    virtual MatrixXd operator()(const IMP::MatrixXd& xlist,
                                bool stupid) const = 0;
     ...
     */
    virtual void add_to_derivatives(const VectorXd& x,
    ...
    virtual Eigen::VectorXd get_derivative_vector(
        unsigned particle_no, const MatrixXd& xlist) const = 0;

    //! for testing purposes
    virtual MatrixXd get_derivative_matrix(const MatrixXd& xlist,
                                           bool stupid) const = 0;
    ...
    virtual kernel::ScalarsTemp get_input_particles() const = 0;
...
class IMPISDEXPORT Linear1DFunction : public UnivariateFunction {
  public:
    ...
    VectorXd operator()(const VectorXd& x) const {
        ...
        VectorXd ret(1, a_val_ * x[0] + b_val_);
        return ret;
    }

    Eigen::VectorXd operator()(const MatrixXd& xlist) const {
    ...
    MatrixXd operator()(const MatrixXd& xlist, bool) const {
        Eigen::VectorXd vec((*this)(xlist));
        MatrixXd ret;
        for (unsigned i = 0; i < xlist.size(); i++)
            ret.push_back(VectorXd(1, vec(i)));
        return ret;
    }

    void add_to_derivatives(const VectorXd& x,
    ...
    MatrixXd get_derivative_matrix(const MatrixXd& xlist, bool) const {
    ...
    MatrixXd get_second_derivative_vector(unsigned particle_a,
                                          unsigned particle_b,
                                          const MatrixXd& xlist, bool) const {
        Eigen::VectorXd mat(
    ...
    kernel::ScalarsTemp get_input_particles() const {
        kernel::ScalarsTemp ret;
        ret.push_back(a_);
        ret.push_back(b_);
        return ret;
    }
...
class IMPISDEXPORT GeneralizedGuinierPorodFunction : public UnivariateFunction {
  ...
  private:
    ...
    base::Pointer<kernel::Scalar> G_, Rg_, d_, s_, A_;
