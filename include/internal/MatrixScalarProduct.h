...
// specialize traits for MatrixScalarProduct
template <class Mat, class Scal>
struct traits<MatrixScalarProduct<Mat, Scal> > {
    static_assert(std::is_same<typename Mat::scalar_type,
                               typename Scal::scalar_type>::value,
                  "cannot mix different scalar types");
    typedef typename Mat::scalar_type scalar_type;
    typedef typename Mat::result_type result_type;
    /*
...
class MatrixScalarProduct
    : public MatrixBase<MatrixScalarProduct<Mat, Scal> > {
   private:
    Mat lhs_;
    Scal rhs_;

   public:
    typedef typename traits<MatrixScalarProduct<Mat, Scal> >::scalar_type
        scalar_type;
    typedef typename traits<MatrixScalarProduct<Mat, Scal> >::result_type
        result_type;
    typedef typename traits<MatrixScalarProduct<Mat, Scal> >::node_type
        node_type;
