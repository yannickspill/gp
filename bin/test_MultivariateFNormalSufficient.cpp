...
bool test_2D() {
...

    {
        // density
        double observed = mv->density();
        double expected = 1 / (2 * PI * sigma1 * sigma2 * sqrt(1 - rho * rho)) *
                          exp(-1 / (2 * (1 - rho * rho)) *
                              (SQUARE(FA(0, 0) - FM(0)) / (sigma1 * sigma1) +
                               SQUARE(FA(0, 1) - FM(1)) / (sigma2 * sigma2) -
                               2 * rho * (FA(0, 0) - FM(0)) *
...
bool test_factor() {
    ...
    NEWPTR(MultivariateFNormalSufficient, mv,
           (FA, JA, FM, factor * factor * Sigma));
    NEWPTR(MultivariateFNormalSufficient, mvf, (FA, JA, FM, Sigma, factor));
    // mv->set_was_used(true);
    ...
    NEWPTR(MultivariateFNormalSufficient, mv2,
           (FA, JA, FM, factor * factor * Sigma));
