...
MultivariateFNormalSufficient::MultivariateFNormalSufficient(
    const MatrixXd& FX, double JF, const VectorXd& FM, const MatrixXd& Sigma,
    double factor) {
    // O(1)
    reset_flags();
    N_ = FX.rows();
    M_ = FX.cols();
    LOG("MVN: direct init with N=" << N_ << " and M=" << M_ << std::endl);
...
MatrixXd MultivariateFNormalSufficient::evaluate_second_derivative_Sigma_Sigma(
    ...
    MatrixXd ret(0.5 *
                 (-P.col(n) * P.row(m) + Peps(n) * (tmp + tmp.transpose())));
    return ret / SQUARE(factor_);
}
...
Eigen::LDLT<MatrixXd, Eigen::Upper> MultivariateFNormalSufficient::get_ldlt()
    ...
    if (!flag_ldlt_) {
        ...
        CHECK(ldlt.isPositive(), "Sigma matrix is not positive semidefinite!");
