#include "GaussianProcessInterpolation.h"
#include "macros.h"
#include <boost/scoped_ptr.hpp>

GaussianProcessInterpolation::GaussianProcessInterpolation(
    MatrixXd x, VectorXd sample_mean, VectorXd sample_std, unsigned n_obs,
...
void GaussianProcessInterpolation::compute_I(VectorXd mean) {
    I_ = VectorXd(M_);
    IMP_LOG_TERSE("I: ");
...
void GaussianProcessInterpolation::compute_S(VectorXd std) {
...
MatrixXd GaussianProcessInterpolation::get_data_abscissa() const {
    return x_;
}

VectorXd GaussianProcessInterpolation::get_data_mean() const {
    VectorXd ret;
    VectorXd I(get_I());
...
MatrixXd GaussianProcessInterpolation::get_data_variance() const {
    MatrixXd ret;
    MatrixXd S(get_S());
    for (unsigned i = 0; i < M_; i++) {
        VectorXd val;
...
double GaussianProcessInterpolation::get_posterior_mean(VectorXd x) const {
...
VectorXd GaussianProcessInterpolation::get_wx_vector(VectorXd xval) const {
    const_cast<GaussianProcessInterpolation *>(this)->update_flags_covariance();
...
VectorXd GaussianProcessInterpolation::get_wx_vector_derivative(
    VectorXd xval, unsigned p) const {
    const_cast<GaussianProcessInterpolation *>(this)->update_flags_covariance();
...
VectorXd GaussianProcessInterpolation::get_wx_vector_second_derivative(
    VectorXd xval, unsigned i, unsigned j) const {
    const_cast<GaussianProcessInterpolation *>(this)->update_flags_covariance();
    unsigned nm = get_number_of_m_particles();
    // derivative wrt mean particles and/or sigma is zero
...
VectorXd GaussianProcessInterpolation::get_m_derivative(unsigned particle)
const {
    const_cast<GaussianProcessInterpolation *>(this)->update_flags_mean();
...
MatrixXd GaussianProcessInterpolation::get_Omega_derivative(unsigned particle)
const {
    const_cast<GaussianProcessInterpolation *>(this)->update_flags_covariance();
...
Eigen::LDLT<MatrixXd, Eigen::Upper> GaussianProcessInterpolation::get_ldlt()
const {
...
VectorXd GaussianProcessInterpolation::get_dcov_dwq(VectorXd q) const {
    VectorXd wq(get_wx_vector(q));
    Eigen::LDLT<MatrixXd, Eigen::Upper> ldlt(get_ldlt());
    return -2 * ldlt.solve(wq);
}

MatrixXd GaussianProcessInterpolation::get_dcov_dOm(VectorXd q) const {
    VectorXd wq(get_wx_vector(q));
    Eigen::LDLT<MatrixXd, Eigen::Upper> ldlt(get_ldlt());
...
MatrixXd GaussianProcessInterpolation::get_d2cov_dwq_dOm(VectorXd q,
        unsigned m) const {
    VectorXd wq(get_wx_vector(q));
...
MatrixXd GaussianProcessInterpolation::get_d2cov_dOm_dOm(VectorXd q, unsigned m,
        unsigned n) const {
    VectorXd wq(get_wx_vector(q));
    VectorXd L(get_ldlt().solve(wq));
    MatrixXd Omi(get_Omi());
...
/*
  MatrixXd GaussianProcessInterpolation::get_posterior_mean_hessian(VectorXd x,
          unsigned p1, unsigned p2) const
{
  ...
  MatrixXd GaussianProcessInterpolation::get_posterior_covariance_hessian(
          VectorXd x, unsigned p1, unsigned p2) const
{
    return MatrixXd::Zero(1,1);
}
*/
